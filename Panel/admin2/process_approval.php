...
if (isset($_GET['decline'])) {
	...
	$db -> query("DELETE FROM staff_registration WHERE sno='$ID'") or die($db -> error());
...
if (isset($_GET['approve'])) {
	...
	$result = $db -> query("SELECT * FROM staff_registration WHERE sno='$ID'") or die($db -> error());
    $row = $result -> fetch_array();
    $name = $row["Name"];
    $email = $row["email"];
    $department = $row["department"];
    $role = $row["primitiverole"];
    $username = $row["username"];
    $password = $row["password"];

    $db -> query("INSERT INTO staff(Name,Role1,Role2,Dept,email,log_id,pass) values('$name','STAFF','$role','$department','$email','$username','$password')");
