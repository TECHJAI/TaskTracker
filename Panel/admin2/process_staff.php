...
if (isset($_POST['save'])) {
	...
	$Dep1 = $temp_row['Dept'];
	$Dep1 = "/".$Dep1;
	$Dep1 = mysqli_real_escape_string($db,$Dep1);

	$To2 = mysqli_real_escape_string($db,$_POST['Assign_To']);
	$to_res = $db -> query("SELECT * FROM staff WHERE Name='$To2'");
	...
	if(strtotime($Begins)<=strtotime($Ends))
	{
		if($To2 == 'staff')
		{
			...
			if($result->num_rows>0)
			{
				while ($row = $result->fetch_assoc()) 
				{
					...
					$Dep3 = $row['Dept'];
					$Dep3 = "/".$Dep3;
					$Dep3 = mysqli_real_escape_string($db,$Dep3);
					$list2[$num] = $Dep3;
					$list3[$num] = $row['Role2'];
					$list4[$num] = $row['email'];
					$num++;
				...
				for($i = 0;$i<$num1;$i++)
				{
					$db -> query("INSERT INTO tasks(From1,Role1,Dep1,To2,Role2,Dep2,Task,Begins,Ends,Remarks) values('$From1','$Role1','$Dep1','$list1[$i]','$list3[$i]','$list2[$i]','$Task','$Begins','$Ends','$Remarks')") or die($db -> error());
		...
		{
			$self = "Self";
			$db -> query("INSERT INTO tasks(From1,Role1,Dep1,To2,Role2,Dep2,Task,Begins,Ends,Remarks) values('$From1','$Role1','$Dep1','$self','','','$Task','$Begins','$Ends','$Remarks')") or die($db -> error());
		}
		else
		{
			$db -> query("INSERT INTO tasks(From1,Role1,Dep1,To2,Role2,Dep2,Task,Begins,Ends,Remarks) values('$From1','$Role1','$Dep1','$To2','$Role2','$Dep2','$Task','$Begins','$Ends','$Remarks')") or die($db -> error());
