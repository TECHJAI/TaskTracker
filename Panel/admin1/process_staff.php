...
if (isset($_POST['save'])) {
	...
	if(strtotime($Begins)<=strtotime($Ends))
	{
		...
		{
			$tem = mysqli_real_escape_string($mysqli,$item);
			$tem_res = $mysqli -> query("SELECT * FROM staff WHERE Name='$tem'");
			$tem_row = $tem_res -> fetch_array();
			$Role2 = $tem_row['Role2'];
			$Dep2 = $tem_row['Dept'];
			$Dep2 = "/".$Dep2;
			$Dep2 = mysqli_real_escape_string($mysqli,$Dep2);
			$mysqli -> query("INSERT INTO tasks(From1,Role1,Dep1,To2,Role2,Dep2,Task,Begins,Ends,Remarks) values('$From1','$Role1','$Dep1','$item','$Role2','$Dep2','$Task','$Begins','$Ends','$Remarks')");
...
if (isset($_POST['update'])) {
	...
	$mysqli -> query("UPDATE tasks SET From1 = '$From1',Role1 = '$Role1',Dep1 = '$Dep1',Task = '$Task',Begins = '$Begins',Ends = '$Ends',Remarks = '$Remarks' WHERE ID='$ID'") or die($mysqli -> error());
