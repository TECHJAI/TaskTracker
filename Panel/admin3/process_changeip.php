...
if (isset($_POST['save'])) {
	...
	$result = $db -> query("SELECT * FROM staff WHERE log_id='$admin_id'") or die($db -> error());
    ...
    elseif($result4->num_rows>0) {
		...
		if ($o_apass==$apass) 
		{
			$db -> query("UPDATE staff SET log_id = '$aname',pass = '$apass' WHERE log_id='$admin_id'") or die($db -> error());
			echo "<script>
